...
Customer groups allow you to segment customers into categories.

### Fields

| Field | Type | Description |
|-------|------|-------------|
| `id` | ID | Unique group identifier (e.g., `cg_01ABC...`) |
| `name` | String | Group display name |
| `key` | String | Unique key identifier (e.g., `vip`, `enterprise`) |
| `color` | String | Hex color code (e.g., `#3B82F6`) |
...
Many update mutations use `StringInput` for optional string fields:
```json
{ "value": "new value" }
```
...
- **CustomerIdentifierInput**: `{ "customerId": "c_..." }` or `{ "emailAddress": "..." }`
- **CompanyIdentifierInput**: `{ "companyId": "co_..." }` or `{ "domainName": "..." }`
- **TenantIdentifierInput**: `{ "externalId": "..." }` or `{ "tenantId": "te_..." }`
- **TierIdentifierInput**: `{ "externalId": "..." }` or `{ "tierId": "tier_..." }`
- **TierMemberIdentifierInput**: `{ "tenantId": "te_..." }` or `{ "companyId": "co_..." }`
